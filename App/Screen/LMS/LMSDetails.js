...
const LMSDetails = ({ route }) => {
    ...
    const [pdfImages, setPdfImages] = useState([]); // [{uri,width,height}]
    const [pdfImagesLoading, setPdfImagesLoading] = useState(false);
    const [pdfUriForClose, setPdfUriForClose] = useState(""); // the uri used by PdfPageImage
    ...
    const buildPdfUriCandidates = (path) => {
        if (!path) return [];
        const p = String(path);

        // already a url or file uri
        if (/^https?:\/\//i.test(p) || p.startsWith("file://")) return [p, p.replace(/^file:\/\//, "")];
    ...
    useEffect(() => {
        ...
        const renderAllPages = async () => {
            if (!securePdfVisible || !securePdfPath) return;
            ...
            try {
                const candidates = buildPdfUriCandidates(securePdfPath);

                let images = null;
                let usedUri = "";

                // try both "file://..." and raw path (different libs/platforms prefer different formats)
                for (const uri of candidates) {
                    try {
                        // scale controls sharpness vs memory; 1.5–2 is usually good
                        const scale = 1.8;
                        const result = await PdfPageImage.generateAllPages(uri, scale);
                ...
                if (!images) {
                    throw new Error("Unable to render PDF pages.");
                }

                if (!alive) return;

                setPdfImages(images);
                setPdfUriForClose(usedUri);
            } catch (e) {
                if (!alive) return;
                Alert.alert("PDF error", e?.message || "Unable to load PDF");
            } finally {
                if (alive) setPdfImagesLoading(false);
            }
        };
    ...
    }, [securePdfVisible, securePdfPath]);

    const closeSecurePdf = useCallback(async () => {
        try {
            setSecurePdfVisible(false);

            // cleanup images generated by PdfPageImage
            if (pdfUriForClose) {
                try { await PdfPageImage.close(pdfUriForClose); } catch (_) { }
            }

            // remove the downloaded pdf from cache
    ...
    }, [securePdfPath, pdfUriForClose]);

    const pdfDocHeight = useMemo(() => {
        if (!pdfImages?.length) return 0;

        const gap = 12;
        const padTop = 8;
        const padBottom = 24;

        return pdfImages.reduce((sum, img) => {
            const w = Number(img?.width || 1);
            const h = Number(img?.height || 1);
            const scaledH = Math.max(240, Math.round((width * h) / w));
            return sum + scaledH + gap;
        }, padTop + padBottom);
    }, [pdfImages, width]);

    const pdfViewportHeight = useMemo(() => {
        // Modal height minus header
        return Math.max(1, height - pdfHeaderHeight);
    }, [height, pdfHeaderHeight]);

    // Shift from "centered" start to "top" start
    const initialOffsetY = useMemo(() => {
        const delta = (pdfDocHeight - pdfViewportHeight) / 2;
        return delta > 0 ? delta : 0;
    }, [pdfDocHeight, pdfViewportHeight]);
        ...
        <View style={{ flex: 1, backgroundColor: colors.background }}>
            ...
            >
                <View style={{ flex: 1, backgroundColor: "#0B1220" }}>
                    {/* Header */}
                    <View
                        onLayout={(e) => setPdfHeaderHeight(e.nativeEvent.layout.height)}
                        style={{
                    ...
                    {/* PDF body (full-document pinch zoom) */}
                    {securePdfPath ? (
                        pdfImagesLoading ? (
                            <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
                                <ActivityIndicator size="large" />
                                <Text style={{ marginTop: 10, color: "#cbd5e1" }}>Preparing pages…</Text>
                            </View>
                        ) : pdfImages.length ? (
                            <ReactNativeZoomableView
                                ...
                                style={{ flex: 1 }}
                                contentWidth={width}
                                contentHeight={pdfDocHeight}
                            >
                                <View style={{ width, paddingTop: 8, paddingBottom: 24 }}>
                                    {pdfImages.map((img, idx) => {
                                        ...
                                        return (
                                            <Image
                                                key={String(idx)}
                                                source={{ uri: img.uri }}
                                                style={{ width, height: scaledH, marginBottom: 12 }}
                                                resizeMode="contain"
                                            />
                                        );
                                    })}
                            ...
                            <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
                                <Text style={{ color: "#cbd5e1" }}>No pages to display.</Text>
                            </View>
                        )
                    ) : (
                        <View style={{ flex: 1, alignItems: "center", justifyContent: "center" }}>
